...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (initialTab) {
        ...
        // 学生数据
        const parameter = [
            { values: [85, 75, 90, 65, 70], color: '#FF6384' },
            { values: [70, 85, 80, 75, 60], color: '#36A2EB' },
            { values: [90, 60, 75, 85, 80], color: '#4BC0C0' }
        ];

        // 为每个容器初始化雷达图
        parameter.forEach((parameter, index) => {
            const chart = echarts.init(document.getElementById(`radar${index + 1}`));
            chart.setOption({
                ...
                series: [{
                    type: 'radar',
                    data: [{ value: parameter.values }],
                    areaStyle: { opacity: 0.3, color: parameter.color },
                    lineStyle: { color: parameter.color }
                }]
            });
        });
        ...
        window.addEventListener('resize', () => {
            parameter.forEach((_, index) => {
                const chart = echarts.getInstanceByDom(document.getElementById(`radar${index + 1}`));
                chart?.resize();
            });
        });